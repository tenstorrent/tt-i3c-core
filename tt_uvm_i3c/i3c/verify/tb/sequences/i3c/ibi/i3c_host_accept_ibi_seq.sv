...
//
// File        : i3c_host_accept_ibi_seq.sv
...
`ifndef I3C_HOST_ACCEPT_IBI_SEQ_SV
`define I3C_HOST_ACCEPT_IBI_SEQ_SV

class i3c_host_broadcast_ccc_seq extends
    uvm_sequence #(i3c_seq_item, i3c_seq_item);
    `uvm_object_utils(i3c_host_broadcast_ccc_seq)

    bit [7:0] ccc = RSTDAA;

    function new(string name = "i3c_host_broadcast_ccc_seq");
        super.new(name);
    endfunction
        ...
        if (response == null)
            ...
            `uvm_fatal("IBI_CCC_NACK",
                       $sformatf("Target NACKed required broadcast CCC 0x%02h",
                                 ccc))
    endtask
endclass : i3c_host_broadcast_ccc_seq
...
// The bundled/reviewed I3C VIP driver returns one response after the address phase and consumes a
// second item to move from DrvSelectNext through STOP back to DrvIdle.
class i3c_host_bus_idle_seq extends
    uvm_sequence #(i3c_seq_item, i3c_seq_item);
    `uvm_object_utils(i3c_host_bus_idle_seq)

    bit [6:0] unassigned_addr;

    function new(string name = "i3c_host_bus_idle_seq");
        super.new(name);
    endfunction
    ...
    endtask
endclass : i3c_host_bus_idle_seq

class i3c_host_respond_ibi_seq extends
    uvm_sequence #(i3c_seq_item, i3c_seq_item);
    `uvm_object_utils(i3c_host_respond_ibi_seq)
    ...
    function new(string name = "i3c_host_respond_ibi_seq");
        super.new(name);
    endfunction
    ...
    endtask
endclass : i3c_host_respond_ibi_seq
...
class i3c_host_respond_ibi_burst_seq extends
    uvm_sequence #(i3c_seq_item, i3c_seq_item);
    `uvm_object_utils(i3c_host_respond_ibi_burst_seq)
    ...
    function new(string name = "i3c_host_respond_ibi_burst_seq");
        super.new(name);
    endfunction
    ...
    endtask
endclass : i3c_host_respond_ibi_burst_seq

// Backward-compatible ACK-only sequence used by the basic IBI test.
class i3c_host_accept_ibi_seq extends i3c_host_respond_ibi_seq;
    `uvm_object_utils(i3c_host_accept_ibi_seq)

    function new(string name = "i3c_host_accept_ibi_seq");
        super.new(name);
        ibi_ack = 1'b1;
    endfunction
endclass : i3c_host_accept_ibi_seq

`endif // I3C_HOST_ACCEPT_IBI_SEQ_SV
