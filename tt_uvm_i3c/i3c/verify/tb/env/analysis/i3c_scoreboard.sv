...
//
// File        : i3c_scoreboard.sv
// Description : Generic native-I3C CSR scoreboard.
// Authors     : Huynh Pham Anh Duy, Thai Hai Dang
// Date        : 2026-07-25
//
// *****************************************************************************

class i3c_scoreboard #(int ADDR_WIDTH = TB_ADDR_WIDTH,
                                   int DATA_WIDTH = TB_DATA_WIDTH)
  extends uvm_scoreboard;

    `uvm_component_param_utils(i3c_scoreboard #(ADDR_WIDTH, DATA_WIDTH))
    ...
    uvm_analysis_imp #(
        ...
        i3c_scoreboard #(ADDR_WIDTH, DATA_WIDTH)
    ) bus_export;

    i3c_csr_scoreboard_cfg #(ADDR_WIDTH, DATA_WIDTH) cfg;
    i3c_env_cfg env_cfg;
    ...
    function new(string name = "i3c_scoreboard",
    ...
    function void build_phase(uvm_phase phase);
        ...
        if (!uvm_config_db#(
                i3c_csr_scoreboard_cfg #(ADDR_WIDTH, DATA_WIDTH)
            )::get(this, "", "cfg", cfg))
            `uvm_fatal(get_type_name(), "Missing i3c_csr_scoreboard_cfg")
    endfunction
    ...
    protected function void record_match(string check_name,
        ...
        modeled_checks++;
        match_count++;
        `uvm_info("I3C_SB_MATCH", $sformatf(
    ...
    protected function void record_mismatch(string check_name,
        ...
        modeled_checks++;
        mismatch_count++;
        `uvm_error("I3C_SB_MISMATCH", $sformatf(
    ...
    virtual function void write(axi4lite_item #(ADDR_WIDTH, DATA_WIDTH) item);
        ...
        // The adapter presents native CSR offsets directly; there is no
        // verification-owned absolute base address or access window.
        offset = item.addr;
        case (offset)
            ...
            default: begin
                unmodeled_accesses++;
                `uvm_info("I3C_SB_UNMODELED", $sformatf(
    ...
    function void report_phase(uvm_phase phase);
        ...
        `uvm_info("I3C_SB", "-------------------------------------------------", UVM_LOW)
        `uvm_info("I3C_SB", " I3C CSR SCOREBOARD SUMMARY", UVM_LOW)
        `uvm_info("I3C_SB", $sformatf(" CSR transactions    : %0d", transactions), UVM_LOW)
        `uvm_info("I3C_SB", $sformatf(" CSR modeled checks  : %0d", modeled_checks), UVM_LOW)
        `uvm_info("I3C_SB", $sformatf(" CSR matches         : %0d", match_count), UVM_LOW)
        `uvm_info("I3C_SB", $sformatf(" CSR mismatches      : %0d", mismatch_count), UVM_LOW)
        `uvm_info("I3C_SB", $sformatf(" CSR unmodeled       : %0d", unmodeled_accesses), UVM_LOW)
        `uvm_info("I3C_SB", $sformatf(" Reset events        : %0d", reset_events), UVM_LOW)
        `uvm_info("I3C_SB", $sformatf(" IBI checker enabled : %0b", ibi_checker_enabled), UVM_LOW)
        `uvm_info("I3C_SB", $sformatf(" IBI completions     : %0d", ibi_completions), UVM_LOW)
        `uvm_info("I3C_SB", $sformatf(" IBI mismatches      : %0d", ibi_mismatches), UVM_LOW)
        `uvm_info("I3C_SB", $sformatf(" Total mismatches    : %0d", total_mismatches), UVM_LOW)
        if (total_mismatches != 0)
            `uvm_error("I3C_SB", $sformatf(
            ...
            `uvm_info("I3C_SB", " PASSED: 0 total mismatches", UVM_LOW)
        `uvm_info("I3C_SB", "-------------------------------------------------", UVM_LOW)
    endfunction
