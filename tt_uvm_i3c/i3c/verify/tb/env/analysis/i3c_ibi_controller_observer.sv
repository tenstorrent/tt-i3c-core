...
//
// File        : i3c_ibi_controller_observer.sv
...
`ifndef I3C_IBI_CONTROLLER_OBSERVER_SV
`define I3C_IBI_CONTROLLER_OBSERVER_SV
...
class i3c_ibi_controller_observer extends uvm_component;
  `uvm_component_utils(i3c_ibi_controller_observer)

  uvm_analysis_imp_ibi_ctrl_observer_target #(
    i3c_target_ibi_observation,
    i3c_ibi_controller_observer
  ) target_export;
  uvm_analysis_imp_ibi_ctrl_observer_bus #(
    i3c_item, i3c_ibi_controller_observer
  ) bus_export;
  uvm_analysis_imp_ibi_ctrl_observer_axi #(
    axi4lite_item #(TB_ADDR_WIDTH, TB_DATA_WIDTH),
    i3c_ibi_controller_observer
  ) axi_export;
  uvm_analysis_port #(i3c_ibi_controller_actual_item) actual_port;

  i3c_env_cfg cfg;

  protected i3c_target_ibi_observation target_q[$];
  protected i3c_ibi_passive_item passive_q[$];
  protected i3c_ibi_controller_actual_item completed_q[$];
  protected i3c_ibi_controller_actual_item active_hci;
  ...
  function new(string name = "i3c_ibi_controller_observer",
  ...
  protected function void attach_target(
      ...
      i3c_target_ibi_observation target);
  ...
  virtual function void write_ibi_ctrl_observer_axi(
    ...
    if (active_hci == null) begin
      active_hci = i3c_ibi_controller_actual_item::type_id::create(
        "actual");
...
endclass : i3c_ibi_controller_observer

`endif // I3C_IBI_CONTROLLER_OBSERVER_SV
