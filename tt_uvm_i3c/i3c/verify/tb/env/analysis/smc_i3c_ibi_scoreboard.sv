...
`ifndef SMC_I3C_IBI_SCOREBOARD_SV
`define SMC_I3C_IBI_SCOREBOARD_SV

`uvm_analysis_imp_decl(_ibi_sb_expected)
`uvm_analysis_imp_decl(_ibi_sb_actual)
`uvm_analysis_imp_decl(_ibi_sb_axi)

class smc_i3c_ibi_scoreboard extends uvm_scoreboard;
  `uvm_component_utils(smc_i3c_ibi_scoreboard)

  uvm_analysis_imp_ibi_sb_expected #(
    smc_i3c_ibi_expected_item, smc_i3c_ibi_scoreboard
  ) expected_export;
  uvm_analysis_imp_ibi_sb_actual #(
    smc_i3c_ibi_actual_item, smc_i3c_ibi_scoreboard
  ) actual_export;
  uvm_analysis_imp_ibi_sb_axi #(
    axi4lite_item #(TB_ADDR_WIDTH, TB_DATA_WIDTH), smc_i3c_ibi_scoreboard
  ) axi_export;
  ...
  function new(string name = "smc_i3c_ibi_scoreboard",
  ...
  protected function string actual_key(smc_i3c_ibi_actual_item item);
    return $sformatf("%0d:%02h:%0d", item.source_instance,
  ...
  protected function void emit_queue_coverage(bit [2:0] status_source,
    ...
    item = smc_i3c_ibi_item::type_id::create("queue_irq_item");
  ...
  protected function void emit_sva_completion(
    ...
    item = smc_i3c_ibi_item::type_id::create("target_sva_completion");
  ...
  virtual function void write_ibi_sb_expected(
    ...
    if (item == null)
      return;
    expected = smc_i3c_ibi_expected_item::type_id::create("expected_copy");
    expected.copy(item);
  ...
  virtual function void write_ibi_sb_actual(smc_i3c_ibi_actual_item item);
    smc_i3c_ibi_actual_item actual;

    if (item == null)
      return;
    actual = smc_i3c_ibi_actual_item::type_id::create("actual_copy");
    actual.copy(item);
...
endclass : smc_i3c_ibi_scoreboard

`endif // SMC_I3C_IBI_SCOREBOARD_SV
