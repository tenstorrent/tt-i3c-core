...
//
// File        : smc_i3c_ibi_recovery_checker.sv
...
`ifndef SMC_I3C_IBI_RECOVERY_CHECKER_SV
`define SMC_I3C_IBI_RECOVERY_CHECKER_SV

`uvm_analysis_imp_decl(_ibi_recovery_event)
`uvm_analysis_imp_decl(_ibi_recovery_axi)

class smc_i3c_ibi_recovery_checker extends uvm_component;
  `uvm_component_utils(smc_i3c_ibi_recovery_checker)

  uvm_analysis_imp_ibi_recovery_event #(
    smc_i3c_ibi_item, smc_i3c_ibi_recovery_checker
  ) event_export;
  uvm_analysis_imp_ibi_recovery_axi #(
    axi4lite_item #(TB_ADDR_WIDTH, TB_DATA_WIDTH),
    smc_i3c_ibi_recovery_checker
  ) axi_export;
  uvm_analysis_port #(smc_i3c_ibi_item) recovery_port;
  ...
  function new(string name = "smc_i3c_ibi_recovery_checker",
  ...
  protected function void publish_result(bit recovered);
    ...
    if (!active)
      return;
    item = smc_i3c_ibi_item::type_id::create("recovery_result");
  ...
  virtual function void write_ibi_recovery_event(smc_i3c_ibi_item item);
...
`endif
  ...
  endtask
endclass : smc_i3c_ibi_recovery_checker

`endif // SMC_I3C_IBI_RECOVERY_CHECKER_SV
