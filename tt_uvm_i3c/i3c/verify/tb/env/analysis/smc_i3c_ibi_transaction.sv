...
`ifndef SMC_I3C_IBI_TRANSACTION_SV
`define SMC_I3C_IBI_TRANSACTION_SV
...
class smc_i3c_ibi_expected_item extends uvm_sequence_item;
...
endclass : smc_i3c_ibi_expected_item

class smc_i3c_ibi_actual_item extends uvm_sequence_item;
  longint unsigned transaction_id;
  bit [2:0] source_instance;
  longint unsigned source_sequence;
  bit [6:0] addr;
  ...
  `uvm_object_utils_begin(smc_i3c_ibi_actual_item)
    `uvm_field_int(transaction_id, UVM_DEFAULT)
    `uvm_field_int(source_instance, UVM_DEFAULT)
    `uvm_field_int(source_sequence, UVM_DEFAULT)
  ...
  function new(string name = "smc_i3c_ibi_actual_item");
    super.new(name);
  endfunction
endclass : smc_i3c_ibi_actual_item

class smc_i3c_ibi_passive_item;
  bit [6:0] addr;
  longint unsigned source_sequence;
  bit ack;
  byte unsigned data[$];
endclass : smc_i3c_ibi_passive_item

class smc_i3c_ibi_controller_actual_item extends uvm_sequence_item;
  longint unsigned transaction_id;
  bit [2:0] source_instance;
  longint unsigned source_sequence;
  bit [6:0] addr;
  ...
  `uvm_object_utils_begin(smc_i3c_ibi_controller_actual_item)
    `uvm_field_int(transaction_id, UVM_DEFAULT)
    `uvm_field_int(source_instance, UVM_DEFAULT)
    `uvm_field_int(source_sequence, UVM_DEFAULT)
    `uvm_field_int(addr, UVM_DEFAULT)
  ...
  function new(string name = "smc_i3c_ibi_controller_actual_item");
    super.new(name);
  endfunction
endclass : smc_i3c_ibi_controller_actual_item

`endif // SMC_I3C_IBI_TRANSACTION_SV
