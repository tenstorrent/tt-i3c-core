...
`ifndef I3C_IBI_PREDICTOR_SV
`define I3C_IBI_PREDICTOR_SV

`uvm_analysis_imp_decl(_ibi_predictor_axi)

class i3c_ibi_predictor extends uvm_component;
  `uvm_component_utils(i3c_ibi_predictor)

  uvm_analysis_imp_ibi_predictor_axi #(
    axi4lite_item #(TB_ADDR_WIDTH, TB_DATA_WIDTH), i3c_ibi_predictor
  ) axi_export;
  uvm_analysis_port #(i3c_ibi_expected_item) expected_port;

  i3c_env_cfg cfg;

  protected i3c_ibi_expected_item payload_fill_item;
  ...
  function new(string name = "i3c_ibi_predictor",
  ...
  virtual function void write_ibi_predictor_axi(
    ...
    if (payload_words_remaining == 0) begin
      payload_fill_item = i3c_ibi_expected_item::type_id::create(
        "expected_item");
...
endclass : i3c_ibi_predictor

`endif // I3C_IBI_PREDICTOR_SV
