...
//
// File        : i3c_controller_hci_helper.sv
...
`ifndef I3C_CONTROLLER_HCI_HELPER_SV
`define I3C_CONTROLLER_HCI_HELPER_SV

class i3c_controller_hci_helper #(
    int ADDR_WIDTH = TB_ADDR_WIDTH,
    int DATA_WIDTH = TB_DATA_WIDTH
) extends uvm_object;
  `uvm_object_param_utils(
    i3c_controller_hci_helper #(ADDR_WIDTH, DATA_WIDTH))

  i3c_csr_helper #(ADDR_WIDTH, DATA_WIDTH) csr_helper;
  i3c_ral_model ral_model;
  i3c_env_cfg cfg;

  function new(string name = "i3c_controller_hci_helper");
    super.new(name);
  endfunction
  ...
  protected function longint unsigned address_to_offset(
    ...
    // The RAL map and AXI adapter both use native CSR offsets directly.
  ...
  endtask
endclass : i3c_controller_hci_helper

`endif // I3C_CONTROLLER_HCI_HELPER_SV
