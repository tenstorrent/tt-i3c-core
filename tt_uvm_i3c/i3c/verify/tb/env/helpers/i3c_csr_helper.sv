...
class i3c_csr_helper #(int ADDR_WIDTH = TB_ADDR_WIDTH,
                           int DATA_WIDTH = TB_DATA_WIDTH) extends uvm_object;

    `uvm_object_param_utils(i3c_csr_helper #(ADDR_WIDTH, DATA_WIDTH))
    ...
    function new(string name = "i3c_csr_helper");
        super.new(name);
    endfunction
    ...
    virtual task read_csr(
        ...
        i3c_csr_access_seq #(ADDR_WIDTH, DATA_WIDTH) seq;

        validate_access(offset);
        seq = i3c_csr_access_seq #(ADDR_WIDTH, DATA_WIDTH)::type_id::create(
            $sformatf("i3c_csr_read_%0h", offset));
        seq.write_access = 1'b0;
        seq.address = ADDR_WIDTH'(offset);
