...
//
// File        : i3c_env.sv
// Description : Top-level UVM environment for native I3C verification.
...
class i3c_env extends uvm_env;
    ...
    i3c_env_cfg       cfg;
    i3c_vseq_context  vseq_ctx;  // 'context' is a SV keyword
    i3c_vseq_services services;
    // Keep a distinct type alias because the object handle intentionally has
    // the historical/public name `i3c_csr_helper`.  Some VCS versions resolve
    // that identifier as the handle at the factory call site and reject the
    // parameterized type syntax (`#(...)`).
    typedef i3c_csr_helper #(TB_ADDR_WIDTH, TB_DATA_WIDTH) i3c_csr_helper_t;
    i3c_csr_helper_t i3c_csr_helper;
    i3c_scoreboard #(TB_ADDR_WIDTH, TB_DATA_WIDTH) scoreboard;
...
`ifdef I3C_DV_NATIVE_RAL
    i3c_ibi_predictor ibi_predictor;
    i3c_ibi_observer ibi_observer;
    i3c_ibi_coverage ibi_functional_cov;
    i3c_ibi_sva_probe ibi_sva_probe;
    i3c_ibi_recovery_checker ibi_recovery_checker;
    i3c_ibi_scoreboard ibi_scoreboard;
    i3c_dv_host_driver ibi_host_driver;
    i3c_dv_target_driver ibi_target_driver;
    i3c_dv_target_driver ibi_target_driver_by_slot[TB_MAX_IBI_REQUESTERS];
    i3c_dv_target_driver ibi_target_driver_secondary;
    i3c_ibi_controller_predictor ibi_controller_predictor;
    i3c_ibi_controller_observer ibi_controller_observer;
    i3c_ibi_controller_scoreboard ibi_controller_scoreboard;
`endif
`endif
`ifdef I3C_DV_NATIVE_RAL
    i3c_ral_model i3c_ral_model;
    typedef i3c_controller_hci_helper #(TB_ADDR_WIDTH, TB_DATA_WIDTH)
      i3c_controller_hci_helper_t;
    i3c_controller_hci_helper_t i3c_controller_hci_helper;
    i3c_axi4lite_reg_adapter #(TB_ADDR_WIDTH, TB_DATA_WIDTH) ral_adapter;
    uvm_reg_predictor #(axi4lite_item #(TB_ADDR_WIDTH, TB_DATA_WIDTH)) ral_predictor;
`endif

    function new(string name = "i3c_env", uvm_component parent = null);
    ...
    function void build_phase(uvm_phase phase);
        ...
        services = i3c_vseq_services::type_id::create("services");
        services.vif = cfg.vif;

        i3c_csr_helper = i3c_csr_helper_t::type_id::create(
            "i3c_csr_helper");

        vseq_ctx = i3c_vseq_context::type_id::create("context");
        vseq_ctx.cfg = cfg;
        vseq_ctx.services = services;
        vseq_ctx.i3c_csr_helper = i3c_csr_helper;
        i3c_ibi_blocked_cov = i3c_ibi_blocked_coverage::type_id::create(
            "i3c_ibi_blocked_cov");
...
`ifdef I3C_DV_NATIVE_RAL
        if (cfg.has_rtl && cfg.has_ral) begin
            ...
            ral_adapter = i3c_axi4lite_reg_adapter #(
            ...
            i3c_controller_hci_helper =
              i3c_controller_hci_helper_t::type_id::create(
                "i3c_controller_hci_helper");
...
`ifdef I3C_DV_NATIVE_RTL
        if (cfg.has_i3c_agent) begin
            ...
            if ((cfg.ibi_controller_target_count < 1) ||
                ...
                `uvm_fatal("I3C_TARGET_COUNT",
                           $sformatf("IBI requester count must be 1..%0d, got %0d",
                                     TB_MAX_IBI_REQUESTERS,
                                     cfg.ibi_controller_target_count))
            if (cfg.i3c_vif == null)
                `uvm_fatal("I3C_VIF", "IBI test requested the I3C agent without an i3c_if")
            if (cfg.i3c_cfg == null)
            ...
            cfg.i3c_cfg.if_mode =
              (cfg.ibi_dut_role == I3C_IBI_DUT_CONTROLLER) ?
                Device : Host;
            ...
            if (cfg.ibi_dut_role == I3C_IBI_DUT_CONTROLLER) begin
                ...
                uvm_config_db#(int unsigned)::set(
                  this, "i3c_agt.driver", "i3c_target_slot", 0);
                uvm_config_db#(i3c_env_cfg)::set(
                  this, "i3c_agt.driver", "i3c_env_cfg", cfg);
            end
...
`ifdef I3C_DV_NATIVE_RAL
        if (cfg.has_ibi_scoreboard) begin
            ...
            ibi_observer = i3c_ibi_observer::type_id::create("ibi_observer", this);
            ibi_observer.cfg = cfg;
        end
        if (cfg.has_ibi_coverage) begin
            ibi_functional_cov = i3c_ibi_coverage::type_id::create(
                "ibi_functional_cov", this);
            ibi_sva_probe = i3c_ibi_sva_probe::type_id::create(
                "ibi_sva_probe", this);
        ...
        if (cfg.has_ibi_recovery_checker) begin
            ...
            ibi_recovery_checker =
              i3c_ibi_recovery_checker::type_id::create(
        ...
        if (cfg.has_ibi_scoreboard) begin
            ...
            ibi_predictor = i3c_ibi_predictor::type_id::create(
                "ibi_predictor", this);
            ibi_predictor.cfg = cfg;
            ibi_scoreboard = i3c_ibi_scoreboard::type_id::create(
                "ibi_scoreboard", this);
...
`endif
        ...
        if (cfg.has_scoreboard) begin
            if (cfg.scoreboard_cfg == null)
                cfg.scoreboard_cfg = i3c_csr_scoreboard_cfg #(
                    TB_ADDR_WIDTH, TB_DATA_WIDTH
                )::type_id::create("scoreboard_cfg");
            cfg.scoreboard_cfg.bus_vif = cfg.bus_vif;
            uvm_config_db#(
                i3c_csr_scoreboard_cfg #(TB_ADDR_WIDTH, TB_DATA_WIDTH)
            )::set(this, "scoreboard", "cfg", cfg.scoreboard_cfg);
            scoreboard = i3c_scoreboard #(
                TB_ADDR_WIDTH, TB_DATA_WIDTH
        ...
        if ((ibi_observer != null) || (ibi_predictor != null) ||
            ...
            if (!$cast(ibi_host_driver, i3c_agt.driver))
                `uvm_fatal("IBI_HOST_DRIVER",
                           "IBI checking requires the i3c_dv_host_driver factory override")
        end
        ...
        if ((ibi_controller_observer != null) ||
            ...
            if (!$cast(ibi_target_driver, i3c_agt.driver))
                `uvm_fatal("IBI_TARGET_DRIVER",
                           "Controller IBI checking requires the i3c_dv_target_driver factory override")
            ...
            for (int unsigned slot = 1;
                ...
                if (!$cast(ibi_target_driver_by_slot[slot],
                    ...
                    `uvm_fatal("IBI_AUX_TARGET_DRIVER",
                               $sformatf("IBI Target slot %0d requires the i3c_dv_target_driver override",
                                         slot))
            end
