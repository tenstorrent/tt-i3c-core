...
`ifndef I3C_TARGET_DRIVER_SV
`define I3C_TARGET_DRIVER_SV

class i3c_target_ibi_observation extends uvm_sequence_item;
  ...
  `uvm_object_utils_begin(i3c_target_ibi_observation)
  ...
  function new(string name = "i3c_target_ibi_observation");
    super.new(name);
  endfunction
endclass : i3c_target_ibi_observation
...
class i3c_dv_target_driver extends i3c_driver;
  `uvm_component_utils(i3c_dv_target_driver)

  uvm_analysis_port #(i3c_target_ibi_observation) ibi_analysis_port;
  ...
  protected i3c_env_cfg i3c_cfg;

  function new(string name = "i3c_dv_target_driver",
  ...
  virtual function void build_phase(uvm_phase phase);
    ...
    if (!uvm_config_db#(int unsigned)::get(
            this, "", "i3c_target_slot", target_slot))
      target_slot = 0;
    ...
    void'(uvm_config_db#(i3c_env_cfg)::get(
            this, "", "i3c_env_cfg", i3c_cfg));
  endfunction

  static function void install_type_override();
    i3c_driver::type_id::set_type_override(i3c_dv_target_driver::get_type());
  endfunction

  static function void install_inst_override(string inst_path);
    i3c_driver::type_id::set_inst_override(
      i3c_dv_target_driver::get_type(), inst_path);
  endfunction

  // Keep the vendored VIP untouched while hardening its Device-mode request
  // lifetime for the native multi-target use case.  The base implementation
  ...
  protected function void publish_ibi_observation(
    ...
    observation = i3c_target_ibi_observation::type_id::create(
      "observation");
  ...
  virtual task drive_device_item(i3c_seq_item req, ref i3c_seq_item rsp);
    ...
    if (is_ibi_address_phase) begin
      if (i3c_cfg != null)
        i3c_cfg.ibi_target_driver_armed[target_slot] = 1'b1;
      armed_event = uvm_event_pool::get_global(
        I3C_TARGET_IBI_ARMED_EVENT);
      armed_event.trigger();
      armed_event = uvm_event_pool::get_global(
        $sformatf("%s_%0d", I3C_TARGET_IBI_SLOT_ARMED_EVENT,
                  target_slot));
      armed_event.trigger();
      if ((i3c_cfg != null) && i3c_cfg.ibi_target_driver_barrier_enable)
        wait(i3c_cfg.ibi_target_driver_release);
    end
        ...
        if (is_ibi_address_phase) begin
          forever begin
            ...
            if (cfg.vif.scl_i === 1'b1) begin
              if (i3c_cfg != null)
                i3c_cfg.ibi_target_driver_start_seen[target_slot] = 1'b1;
              break;
            end
          end
        end
    ...
    if (is_ibi_address_phase && (i3c_cfg != null)) begin
      i3c_cfg.ibi_target_driver_completed[target_slot] = 1'b1;
      if (rsp != null) begin
        i3c_cfg.ibi_target_driver_resolved_addr[target_slot] = rsp.addr;
        i3c_cfg.ibi_target_driver_ack[target_slot] = rsp.dev_ack;
        i3c_cfg.ibi_target_driver_arbitration_won[target_slot] =
          (rsp.addr == req.IBI_ADDR) && rsp.dir;
      end
      i3c_cfg.ibi_target_driver_released[target_slot] =
        ((cfg.vif.device_sda_o === 1'b1) &&
         (cfg.vif.device_sda_pp_en === 1'b0));
    end
  ...
  endtask
endclass : i3c_dv_target_driver

`endif // I3C_TARGET_DRIVER_SV
