...
`ifndef SMC_I3C_MONITOR_SV
`define SMC_I3C_MONITOR_SV
...
class smc_i3c_monitor extends i3c_monitor;
  `uvm_component_utils(smc_i3c_monitor)

  function new(string name = "smc_i3c_monitor",
               uvm_component parent = null);
    super.new(name, parent);
  endfunction

  static function void install_type_override();
    i3c_monitor::type_id::set_type_override(smc_i3c_monitor::get_type());
  endfunction
  ...
  virtual protected task ccc_data(
    ...
    fork : smc_device_to_host_capture
      begin
        forever begin
    ...
    if (data_done && !bus_ended)
      wait (bus_ended);
    disable smc_device_to_host_capture;
