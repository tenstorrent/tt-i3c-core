...
//
// File        : smc_i3c_ibi_controller_rx_mdb_decode_vseq.sv
...
`ifndef SMC_I3C_IBI_CONTROLLER_RX_MDB_DECODE_VSEQ_SV
`define SMC_I3C_IBI_CONTROLLER_RX_MDB_DECODE_VSEQ_SV

typedef enum bit [2:0] {
  SMC_I3C_MDB_USER_DEFINED = 3'b000,
  SMC_I3C_MDB_ERROR        = 3'b001,
  SMC_I3C_MDB_MIPI         = 3'b010,
  SMC_I3C_MDB_NON_MIPI     = 3'b011,
  SMC_I3C_MDB_TIMING       = 3'b100,
  SMC_I3C_MDB_PENDING_READ = 3'b101,
  SMC_I3C_MDB_RESERVED_110 = 3'b110,
  SMC_I3C_MDB_RESERVED_111 = 3'b111
} smc_i3c_mdb_group_e;
...
class smc_i3c_ibi_controller_mdb_base_vseq extends smc_i3c_ibi_base_vseq;
  `uvm_object_utils(smc_i3c_ibi_controller_mdb_base_vseq)

  function new(string name = "smc_i3c_ibi_controller_mdb_base_vseq");
    super.new(name);
  endfunction

`ifdef SMC_USE_I3C_RTL
`ifdef SMC_USE_I3C_RAL
  protected function smc_i3c_mdb_group_e classify_mdb(bit [7:0] mdb);
    return smc_i3c_mdb_group_e'(mdb[7:5]);
  endfunction

  protected function string mdb_group_name(smc_i3c_mdb_group_e group);
    case (group)
      SMC_I3C_MDB_USER_DEFINED: return "user-defined/generic";
      SMC_I3C_MDB_ERROR:        return "error";
      SMC_I3C_MDB_MIPI:         return "MIPI-defined";
      SMC_I3C_MDB_NON_MIPI:     return "non-MIPI";
      SMC_I3C_MDB_TIMING:       return "timing/vendor";
      SMC_I3C_MDB_PENDING_READ: return "pending-read notification";
  ...
  protected task run_mdb_case(
      ...
      input bit [7:0] mdb,
      input smc_i3c_mdb_group_e expected_group,
      input smc_i3c_controller_hci_helper #(
        TB_ADDR_WIDTH, TB_DATA_WIDTH) hci_helper);
    ...
    if (expected_group == SMC_I3C_MDB_PENDING_READ) begin
...
`endif
`endif
endclass : smc_i3c_ibi_controller_mdb_base_vseq

class smc_i3c_ibi_controller_rx_mdb_decode_vseq extends
    smc_i3c_ibi_controller_mdb_base_vseq;
  `uvm_object_utils(smc_i3c_ibi_controller_rx_mdb_decode_vseq)

  localparam int unsigned MDB_CASE_COUNT = 4;

  function new(string name = "smc_i3c_ibi_controller_rx_mdb_decode_vseq");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("IBI_MDB_MODE",
                 "MDB decode test requires DUT_MODE=rtl")
...
`ifdef SMC_USE_I3C_RAL
    begin
      ...
      run_mdb_case(0, 8'h00, SMC_I3C_MDB_USER_DEFINED, hci_helper);
      run_mdb_case(1, 8'h2e, SMC_I3C_MDB_ERROR, hci_helper);
      run_mdb_case(2, 8'h80, SMC_I3C_MDB_TIMING, hci_helper);
      run_mdb_case(3, 8'hb0, SMC_I3C_MDB_PENDING_READ, hci_helper);
...
`else
    `uvm_fatal("IBI_MDB_RAL_BUILD",
               "MDB decode test requires SMC_USE_I3C_RAL")
`endif
`else
    `uvm_fatal("IBI_MDB_RTL_BUILD",
               "MDB decode test requires SMC_USE_I3C_RTL")
`endif
  endtask
endclass : smc_i3c_ibi_controller_rx_mdb_decode_vseq

`endif // SMC_I3C_IBI_CONTROLLER_RX_MDB_DECODE_VSEQ_SV
