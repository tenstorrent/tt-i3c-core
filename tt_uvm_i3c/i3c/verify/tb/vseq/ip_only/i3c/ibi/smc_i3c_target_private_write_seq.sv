...
//
// File        : smc_i3c_target_private_write_seq.sv
...
`ifndef SMC_I3C_TARGET_PRIVATE_WRITE_SEQ_SV
`define SMC_I3C_TARGET_PRIVATE_WRITE_SEQ_SV

// One deterministic Device-mode response to a Controller private write. The
// pinned VIP returns after address sampling, then consumes a second item to
// drive ACK and receive the data/T-bit through STOP.
class smc_i3c_target_private_write_seq extends
    uvm_sequence #(i3c_seq_item, i3c_seq_item);
  `uvm_object_utils(smc_i3c_target_private_write_seq)
  ...
  function new(string name = "smc_i3c_target_private_write_seq");
    super.new(name);
  ...
  endtask
endclass : smc_i3c_target_private_write_seq

`endif // SMC_I3C_TARGET_PRIVATE_WRITE_SEQ_SV
