...
//
// File        : i3c_ibi_target_tx_invalid_target_vseq.sv
...
`ifndef I3C_IBI_TARGET_TX_INVALID_TARGET_VSEQ_SV
`define I3C_IBI_TARGET_TX_INVALID_TARGET_VSEQ_SV

class i3c_ibi_target_tx_invalid_target_vseq extends i3c_ibi_base_vseq;
  `uvm_object_utils(i3c_ibi_target_tx_invalid_target_vseq)

  // Directly programming inconsistent identity/capability fields is outside
  // this test's runtime eligibility contract. The reviewed native core accepts any
  ...
  function new(string name = "i3c_ibi_target_tx_invalid_target_vseq");
    super.new(name);
  endfunction

`ifdef I3C_DV_NATIVE_RTL
`ifdef I3C_DV_NATIVE_RAL
  protected task reset_ibi_queue(input i3c_ral_model ral,
  ...
  protected task prove_no_bus_ibi(input i3c_ral_model ral,
  ...
  protected task queue_ineligible_request(input i3c_ral_model ral,
                                           input string operation,
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("IBI_INVALID_MODE",
                 "i3c_ibi_target_tx_invalid_target_test requires native I3C RTL mode")
    if (vseq_ctx.cfg.ibi_dut_role != I3C_IBI_DUT_TARGET)
      `uvm_fatal("IBI_INVALID_ROLE",
                 "i3c_ibi_target_tx_invalid_target_test requires DUT-target/UVM-controller role")
...
`else
    `uvm_fatal("IBI_INVALID_RAL_BUILD",
               "i3c_ibi_target_tx_invalid_target_test requires I3C_DV_NATIVE_RAL")
`endif
`else
    `uvm_fatal("IBI_INVALID_RTL_BUILD",
               "i3c_ibi_target_tx_invalid_target_test requires I3C_DV_NATIVE_RTL")
`endif
  endtask
endclass : i3c_ibi_target_tx_invalid_target_vseq

`endif // I3C_IBI_TARGET_TX_INVALID_TARGET_VSEQ_SV
