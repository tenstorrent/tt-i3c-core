...
//
// File        : smc_i3c_ibi_target_tx_overflow_vseq.sv
...
`ifndef SMC_I3C_IBI_TARGET_TX_OVERFLOW_VSEQ_SV
`define SMC_I3C_IBI_TARGET_TX_OVERFLOW_VSEQ_SV

class smc_i3c_ibi_target_tx_overflow_vseq extends smc_i3c_ibi_base_vseq;
  `uvm_object_utils(smc_i3c_ibi_target_tx_overflow_vseq)

  localparam bit [31:0] OVERFLOW_SENTINEL = 32'hdeadc0de;
  localparam string TARGET_IBI_FIFO_PATH =
    "tb_smc_peripherals_top.u_dut.u_i3c0.u_i3c_core.i3c.xrecovery_handler.ibi_queue.fifo.gen_normal_fifo";

  function new(string name = "smc_i3c_ibi_target_tx_overflow_vseq");
    super.new(name);
  endfunction

`ifdef SMC_USE_I3C_RTL
`ifdef SMC_USE_I3C_RAL
  protected task reset_target_ibi_queue(input smc_i3c_ral_model ral);
  ...
  protected task run_target_tti_overflow(input smc_i3c_ral_model ral);
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("IBI_OVERFLOW_MODE",
                 "smc_i3c_ibi_target_tx_overflow_test requires DUT_MODE=rtl")

`ifdef SMC_USE_I3C_RTL
`ifdef SMC_USE_I3C_RAL
    if (vseq_ctx.cfg.ibi_dut_role == SMC_I3C_IBI_DUT_TARGET) begin
      ...
      `uvm_fatal("IBI_OVERFLOW_SEQUENCE_ROLE",
                 "smc_i3c_ibi_target_tx_overflow_vseq requires DUT Target-TX role")
`else
    `uvm_fatal("IBI_OVERFLOW_RAL_BUILD",
               "smc_i3c_ibi_target_tx_overflow_test requires SMC_USE_I3C_RAL")
`endif
`else
    `uvm_fatal("IBI_OVERFLOW_RTL_BUILD",
               "smc_i3c_ibi_target_tx_overflow_test requires SMC_USE_I3C_RTL")
`endif
  endtask
endclass : smc_i3c_ibi_target_tx_overflow_vseq

`endif // SMC_I3C_IBI_TARGET_TX_OVERFLOW_VSEQ_SV
