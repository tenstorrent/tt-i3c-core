...
//
// File        : i3c_ibi_controller_rx_timing_context_vseq.sv
...
`ifndef I3C_IBI_CONTROLLER_RX_TIMING_CONTEXT_VSEQ_SV
`define I3C_IBI_CONTROLLER_RX_TIMING_CONTEXT_VSEQ_SV

class i3c_ibi_controller_rx_timing_context_vseq extends
    i3c_ibi_base_vseq;
  `uvm_object_utils(i3c_ibi_controller_rx_timing_context_vseq)
  ...
  protected bit busy_start_blocked_evidence;
  protected bit busy_transfer_preserved_evidence;

  covergroup timing_context_cg with function sample(
      bit [1:0] timing_context,
      bit request_while_busy,
      bit deferred);
    option.per_instance = 1;
    cp_context: coverpoint timing_context {
  ...
  function new(
      string name = "i3c_ibi_controller_rx_timing_context_vseq");
    super.new(name);
...
`ifdef I3C_DV_NATIVE_RAL
  ...
  protected task run_normal_write(
      ...
      input string operation);
    i3c_target_private_write_seq target_seq;
    ...
    target_seq = i3c_target_private_write_seq::type_id::create(
    ...
    vseq_ctx.cfg.vif.i3c_controller_ibi_sva_suppress = 1'b1;
    ...
    if (request_after_stop)
      `uvm_info("IBI_TIMING_MARKER",
                $sformatf("%s: IBI request became pending immediately after STOP at %0t",
                        operation, $time),
                UVM_HIGH)
    // Keep suppression asserted beyond the STOP detector's picosecond filter
    // before re-enabling IBI-only properties for the next autonomous request.
    @(posedge vseq_ctx.cfg.vif.clk);
    vseq_ctx.cfg.vif.i3c_controller_ibi_sva_suppress = 1'b0;
  ...
  protected task run_autonomous_ibi(
    ...
    vseq_ctx.cfg.ibi_bus_state =
      (timing_context == TIMING_IDLE) ? 2'd0 : 2'd2;
    vseq_ctx.cfg.vif.i3c_controller_expected_ack = 1'b1;
    if (vseq_ctx.cfg.vif.i3c_controller_ibi_sva_suppress)
      `uvm_fatal("IBI_TIMING_SVA_SUPPRESS",
                 $sformatf("%s attempted autonomous IBI while Controller IBI SVA remained suppressed",
                           operation))
    ...
    // ACK and HCI correctness are checked above. Do not cover checker
    // verdicts: their negative bins can only be sampled by a failing test.
    timing_context_cg.sample(timing_context, request_while_busy, deferred);
    if (request_while_busy && (vseq_ctx.i3c_ibi_blocked_cov != null))
      vseq_ctx.i3c_ibi_blocked_cov.sample_blocked(
        1'b1, 1'b1, busy_start_blocked_evidence,
        busy_transfer_preserved_evidence, deferred,
        target_done &&
  ...
  protected task run_busy_defer_context();
    int unsigned no_start_before;
    int unsigned normal_xfer_before;

    no_start_before = vseq_ctx.cfg.ibi_timing_no_start_busy_checks;
    normal_xfer_before = vseq_ctx.cfg.ibi_timing_normal_xfer_checks;
    run_normal_write(8'h53, 4'h5, 1'b1, 1'b0,
                     "BUSY_DEFER normal write");
    busy_start_blocked_evidence =
      (vseq_ctx.cfg.ibi_timing_no_start_busy_checks == no_start_before + 1);
    busy_transfer_preserved_evidence =
      (vseq_ctx.cfg.ibi_timing_normal_xfer_checks == normal_xfer_before + 1);
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("IBI_TIMING_MODE",
                 "Controller timing-context test requires native I3C RTL mode")
...
`ifdef I3C_DV_NATIVE_RAL
    ...
    vseq_ctx.cfg.ibi_timing_retry_after_taval_samples = 0;
    busy_start_blocked_evidence = 1'b0;
    busy_transfer_preserved_evidence = 1'b0;
...
`else
    `uvm_fatal("IBI_TIMING_RAL_BUILD",
               "Controller timing-context test requires I3C_DV_NATIVE_RAL")
`endif
`else
    `uvm_fatal("IBI_TIMING_RTL_BUILD",
               "Controller timing-context test requires I3C_DV_NATIVE_RTL")
`endif
  endtask
endclass : i3c_ibi_controller_rx_timing_context_vseq

`endif // I3C_IBI_CONTROLLER_RX_TIMING_CONTEXT_VSEQ_SV
