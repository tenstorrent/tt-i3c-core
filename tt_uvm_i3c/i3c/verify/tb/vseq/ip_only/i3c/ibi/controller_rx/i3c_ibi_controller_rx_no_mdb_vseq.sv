...
//
// File        : i3c_ibi_controller_rx_no_mdb_vseq.sv
...
`ifndef I3C_IBI_CONTROLLER_RX_NO_MDB_VSEQ_SV
`define I3C_IBI_CONTROLLER_RX_NO_MDB_VSEQ_SV

class i3c_ibi_controller_rx_no_mdb_vseq extends i3c_ibi_base_vseq;
  `uvm_object_utils(i3c_ibi_controller_rx_no_mdb_vseq)

  function new(string name = "i3c_ibi_controller_rx_no_mdb_vseq");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("IBI_NO_MDB_MODE",
                 "Positive no-MDB reproduction requires native I3C RTL mode")
...
`else
    `uvm_fatal("IBI_NO_MDB_RAL_BUILD",
               "Positive no-MDB reproduction requires I3C_DV_NATIVE_RAL")
`endif
`else
    `uvm_fatal("IBI_NO_MDB_RTL_BUILD",
               "Positive no-MDB reproduction requires I3C_DV_NATIVE_RTL")
`endif
  endtask
endclass : i3c_ibi_controller_rx_no_mdb_vseq

`endif // I3C_IBI_CONTROLLER_RX_NO_MDB_VSEQ_SV
