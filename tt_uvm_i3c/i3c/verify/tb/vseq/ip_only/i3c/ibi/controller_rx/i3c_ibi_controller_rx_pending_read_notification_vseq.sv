...
//
// File        : i3c_ibi_controller_rx_pending_read_notification_vseq.sv
...
`ifndef I3C_IBI_CONTROLLER_RX_PENDING_READ_NOTIFICATION_VSEQ_SV
`define I3C_IBI_CONTROLLER_RX_PENDING_READ_NOTIFICATION_VSEQ_SV

class i3c_ibi_controller_rx_pending_read_notification_vseq extends
    i3c_ibi_controller_mdb_base_vseq;
  `uvm_object_utils(i3c_ibi_controller_rx_pending_read_notification_vseq)
  ...
  function new(
      string name = "i3c_ibi_controller_rx_pending_read_notification_vseq");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("IBI_PRN_MODE",
                 "Pending-read notification test requires native I3C RTL mode")
...
`ifdef I3C_DV_NATIVE_RAL
    begin
      i3c_controller_hci_helper #(TB_ADDR_WIDTH, TB_DATA_WIDTH)
        hci_helper;
      i3c_target_send_ibi_seq target_ibi_seq;
      ...
      target_ibi_seq = i3c_target_send_ibi_seq::type_id::create(
...
`else
    `uvm_fatal("IBI_PRN_RAL_BUILD",
               "Pending-read test requires I3C_DV_NATIVE_RAL")
`endif
`else
    `uvm_fatal("IBI_PRN_RTL_BUILD",
               "Pending-read test requires I3C_DV_NATIVE_RTL")
`endif
  endtask
endclass : i3c_ibi_controller_rx_pending_read_notification_vseq

`endif // I3C_IBI_CONTROLLER_RX_PENDING_READ_NOTIFICATION_VSEQ_SV
