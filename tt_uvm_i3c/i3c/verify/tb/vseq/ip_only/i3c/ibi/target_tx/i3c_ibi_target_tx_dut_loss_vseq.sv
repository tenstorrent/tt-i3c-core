...
//
// File        : i3c_ibi_target_tx_dut_loss_vseq.sv
...
`ifndef I3C_IBI_TARGET_TX_DUT_LOSS_VSEQ_SV
`define I3C_IBI_TARGET_TX_DUT_LOSS_VSEQ_SV

class i3c_ibi_target_tx_dut_loss_vseq extends i3c_ibi_base_vseq;
  `uvm_object_utils(i3c_ibi_target_tx_dut_loss_vseq)

  function new(string name = "i3c_ibi_target_tx_dut_loss_vseq");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("IBI_DUT_LOSS_MODE", "DUT-loss test requires native I3C RTL mode")
...
`ifndef I3C_DV_NATIVE_RTL
    `uvm_fatal("IBI_DUT_LOSS_RTL_BUILD",
               "DUT-loss test requires I3C_DV_NATIVE_RTL")
`elsif I3C_DV_NATIVE_RAL
    begin
      ...
      i3c_target_send_ibi_seq quaternary_seq;
      i3c_ibi_target_tx_busy_bus_block_vseq transfer_precursor;
      i3c_host_busy_private_write_seq arbitration_private_seq;
      ...
      if ($test$plusargs("IBI_ARB_PRECEDE_PRIVATE_WRITE")) begin
        ...
        `uvm_info("IBI_ARB_CONTEXT_PRECURSOR",
                  ...
                  UVM_LOW)
        transfer_precursor =
          i3c_ibi_target_tx_busy_bus_block_vseq::type_id::create(
            "transfer_precursor");
      ...
      if (requester_count >= 4) begin
        quaternary_seq = i3c_target_send_ibi_seq::type_id::create(
      ...
      if (arbitrate_after_transfer) begin
        ...
        arbitration_private_seq =
          i3c_host_busy_private_write_seq::type_id::create(
            "arbitration_private_seq");
...
`else
    `uvm_fatal("IBI_DUT_LOSS_RAL_BUILD",
               "DUT-loss test requires I3C_DV_NATIVE_RAL")
`endif
  endtask
endclass : i3c_ibi_target_tx_dut_loss_vseq
