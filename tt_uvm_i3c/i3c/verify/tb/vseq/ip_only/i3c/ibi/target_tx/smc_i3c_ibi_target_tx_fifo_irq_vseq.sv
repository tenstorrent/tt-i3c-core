...
//
// File        : smc_i3c_ibi_target_tx_fifo_irq_vseq.sv
...
`ifndef SMC_I3C_IBI_TARGET_TX_FIFO_IRQ_VSEQ_SV
`define SMC_I3C_IBI_TARGET_TX_FIFO_IRQ_VSEQ_SV

class smc_i3c_ibi_target_tx_fifo_irq_vseq extends smc_i3c_ibi_base_vseq;
  `uvm_object_utils(smc_i3c_ibi_target_tx_fifo_irq_vseq)

  localparam int unsigned MAX_RECORD_DWORDS = 4;

  function new(string name = "smc_i3c_ibi_target_tx_fifo_irq_vseq");
    super.new(name);
  endfunction
...
`ifdef SMC_USE_I3C_RAL
  ...
  protected task queue_ibi_record(
      input smc_i3c_ral_model ral,
      input smc_i3c_host_respond_ibi_burst_seq host_seq,
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("SMC_I3C_MODE",
                 "smc_i3c_ibi_target_tx_fifo_irq_test requires DUT_MODE=rtl")
    if (vseq_ctx.cfg.ibi_dut_role != SMC_I3C_IBI_DUT_TARGET)
      `uvm_fatal("IBI_FIFO_IRQ_ROLE",
                 "smc_i3c_ibi_target_tx_fifo_irq_test covers DUT-target transmit only")
`ifndef SMC_USE_I3C_RTL
    `uvm_fatal("IBI_VIP_BUILD",
               "I3C host VIP is available only in DUT_MODE=rtl")
`elsif SMC_USE_I3C_RAL
    begin
      smc_i3c_ral_model ral;
      smc_i3c_host_respond_ibi_burst_seq host_seq;
      smc_i3c_host_respond_ibi_burst_seq refill_host_seq;
      ...
      refill_host_seq = smc_i3c_host_respond_ibi_burst_seq::type_id::create(
...
`else
    `uvm_fatal("IBI_RAL_BUILD",
               "smc_i3c_ibi_target_tx_fifo_irq_test requires SMC_USE_I3C_RAL")
`endif
  endtask
endclass : smc_i3c_ibi_target_tx_fifo_irq_vseq

`endif // SMC_I3C_IBI_TARGET_TX_FIFO_IRQ_VSEQ_SV
