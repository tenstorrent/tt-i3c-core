...
//
// File        : i3c_ibi_target_tx_basic_vseq.sv
...
`ifndef I3C_IBI_TARGET_TX_BASIC_VSEQ_SV
`define I3C_IBI_TARGET_TX_BASIC_VSEQ_SV

class i3c_ibi_target_tx_basic_vseq extends i3c_ibi_base_vseq;
    `uvm_object_utils(i3c_ibi_target_tx_basic_vseq)

    function new(string name = "i3c_ibi_target_tx_basic_vseq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        if (!vseq_ctx.cfg.has_rtl)
            `uvm_fatal("I3C_MODE",
                       "i3c_ibi_target_tx_basic_test requires native I3C RTL mode")
        if (vseq_ctx.cfg.ibi_dut_role != I3C_IBI_DUT_TARGET)
            `uvm_fatal("IBI_BASIC_ROLE",
                       "i3c_ibi_target_tx_basic_test covers DUT-target transmit only; use the controller-receive test for DUT-controller mode")
...
`elsif I3C_DV_NATIVE_RAL
        begin
            ...
            host_seq = i3c_host_accept_ibi_seq::type_id::create("host_seq");
...
`else
        `uvm_fatal("IBI_RAL_BUILD", "i3c_ibi_target_tx_basic_test requires I3C_DV_NATIVE_RAL")
`endif
    endtask
endclass : i3c_ibi_target_tx_basic_vseq

`endif // I3C_IBI_TARGET_TX_BASIC_VSEQ_SV
