...
//
// File        : smc_i3c_ibi_ack_nack_vseq.sv
...
`ifndef SMC_I3C_IBI_ACK_NACK_VSEQ_SV
`define SMC_I3C_IBI_ACK_NACK_VSEQ_SV

class smc_i3c_ibi_ack_nack_vseq extends smc_i3c_ibi_base_vseq;
  `uvm_object_utils(smc_i3c_ibi_ack_nack_vseq)

  function new(string name = "smc_i3c_ibi_ack_nack_vseq");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("SMC_I3C_MODE",
                 "smc_i3c_ibi_ack_nack_test requires DUT_MODE=rtl")
    if (vseq_ctx.cfg.ibi_dut_role != SMC_I3C_IBI_DUT_TARGET)
      `uvm_fatal("IBI_ACK_NACK_ROLE",
                 "smc_i3c_ibi_ack_nack_test covers DUT-target transmit only")
...
`else
    `uvm_fatal("IBI_RAL_BUILD",
               "smc_i3c_ibi_ack_nack_test requires SMC_USE_I3C_RAL")
`endif
  endtask
endclass : smc_i3c_ibi_ack_nack_vseq

`endif // SMC_I3C_IBI_ACK_NACK_VSEQ_SV
