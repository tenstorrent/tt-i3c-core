...
//
// File        : i3c_ibi_target_tx_busy_bus_block_vseq.sv
...
`ifndef I3C_IBI_TARGET_TX_BUSY_BUS_BLOCK_VSEQ_SV
`define I3C_IBI_TARGET_TX_BUSY_BUS_BLOCK_VSEQ_SV
...
class i3c_host_busy_private_write_seq extends
    uvm_sequence #(i3c_seq_item, i3c_seq_item);
  `uvm_object_utils(i3c_host_busy_private_write_seq)
  ...
  function new(string name = "i3c_host_busy_private_write_seq");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    transfer_done = 1'b1;
  endtask
endclass : i3c_host_busy_private_write_seq
`endif

class i3c_ibi_target_tx_busy_bus_block_vseq extends i3c_ibi_base_vseq;
  `uvm_object_utils(i3c_ibi_target_tx_busy_bus_block_vseq)

  localparam int unsigned PRIVATE_WRITE_BYTES = 16;

  function new(string name = "i3c_ibi_target_tx_busy_bus_block_vseq");
    super.new(name);
  endfunction
...
`ifdef I3C_DV_NATIVE_RAL
  ...
  protected task wait_private_address_ack(
      input i3c_host_busy_private_write_seq private_seq,
  ...
  protected task check_private_write_queues(input i3c_ral_model ral,
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("IBI_BUSY_MODE",
                 "i3c_ibi_target_tx_busy_bus_block_test requires native I3C RTL mode")
    if (vseq_ctx.cfg.ibi_dut_role != I3C_IBI_DUT_TARGET)
      `uvm_fatal("IBI_BUSY_ROLE",
                 "i3c_ibi_target_tx_busy_bus_block_test requires DUT-target/UVM-controller role")

`ifdef I3C_DV_NATIVE_RTL
`ifdef I3C_DV_NATIVE_RAL
    begin
      i3c_ral_model ral;
      i3c_host_busy_private_write_seq private_seq;
      i3c_host_accept_ibi_seq host_seq;
      uvm_reg_data_t descriptor;
      ...
      private_seq = i3c_host_busy_private_write_seq::type_id::create(
        "private_write_seq");
      ...
      host_seq = i3c_host_accept_ibi_seq::type_id::create("host_ibi_seq");
...
`else
    `uvm_fatal("IBI_BUSY_RAL_BUILD",
               "i3c_ibi_target_tx_busy_bus_block_test requires I3C_DV_NATIVE_RAL")
`endif
`else
    `uvm_fatal("IBI_BUSY_RTL_BUILD",
               "i3c_ibi_target_tx_busy_bus_block_test requires I3C_DV_NATIVE_RTL")
`endif
  endtask
endclass : i3c_ibi_target_tx_busy_bus_block_vseq

`endif // I3C_IBI_TARGET_TX_BUSY_BUS_BLOCK_VSEQ_SV
