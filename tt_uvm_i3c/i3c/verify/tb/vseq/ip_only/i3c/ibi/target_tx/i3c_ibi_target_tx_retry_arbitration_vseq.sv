...
//
// File        : i3c_ibi_target_tx_retry_arbitration_vseq.sv
...
`ifndef I3C_IBI_TARGET_TX_RETRY_ARBITRATION_VSEQ_SV
`define I3C_IBI_TARGET_TX_RETRY_ARBITRATION_VSEQ_SV

class i3c_ibi_target_tx_retry_arbitration_vseq extends i3c_ibi_base_vseq;
  `uvm_object_utils(i3c_ibi_target_tx_retry_arbitration_vseq)

  function new(string name = "i3c_ibi_target_tx_retry_arbitration_vseq");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("IBI_RETRY_MODE",
                 "IBI retry/arbitration test requires native I3C RTL mode")
...
`ifndef I3C_DV_NATIVE_RTL
    `uvm_fatal("IBI_RETRY_RTL_BUILD",
               "IBI retry/arbitration test requires I3C_DV_NATIVE_RTL")
...
`else
    `uvm_fatal("IBI_RETRY_RAL_BUILD",
               "IBI retry/arbitration test requires I3C_DV_NATIVE_RAL")
`endif
  endtask
endclass : i3c_ibi_target_tx_retry_arbitration_vseq

`endif // I3C_IBI_TARGET_TX_RETRY_ARBITRATION_VSEQ_SV
