...
//
// File        : i3c_ibi_controller_rx_recovery_cleanup_vseq.sv
...
`ifndef I3C_IBI_CONTROLLER_RX_RECOVERY_CLEANUP_VSEQ_SV
`define I3C_IBI_CONTROLLER_RX_RECOVERY_CLEANUP_VSEQ_SV

class i3c_ibi_controller_rx_recovery_cleanup_vseq extends
    i3c_ibi_base_vseq;
  `uvm_object_utils(i3c_ibi_controller_rx_recovery_cleanup_vseq)

  function new(string name = "i3c_ibi_controller_rx_recovery_cleanup_vseq");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("IBI_CTRL_RECOVERY_MODE",
                 "Controller recovery cleanup requires native I3C RTL mode")
...
`ifdef I3C_DV_NATIVE_RAL
    begin
      i3c_ibi_controller_rx_reset_flush_vseq reset_flush_vseq;
      i3c_ibi_controller_rx_basic_vseq followup_vseq;
      ...
      reset_flush_vseq =
        i3c_ibi_controller_rx_reset_flush_vseq::type_id::create(
          "reset_flush_vseq");
      reset_flush_vseq.vseq_ctx = vseq_ctx;
      reset_flush_vseq.start(null);

      followup_vseq = i3c_ibi_controller_rx_basic_vseq::type_id::create(
        "followup_vseq");
...
`else
    `uvm_fatal("IBI_CTRL_RECOVERY_RAL_BUILD",
               "Controller recovery cleanup requires I3C_DV_NATIVE_RAL")
`endif
`else
    `uvm_fatal("IBI_CTRL_RECOVERY_RTL_BUILD",
               "Controller recovery cleanup requires I3C_DV_NATIVE_RTL")
`endif
  endtask
endclass : i3c_ibi_controller_rx_recovery_cleanup_vseq

`endif // I3C_IBI_CONTROLLER_RX_RECOVERY_CLEANUP_VSEQ_SV
