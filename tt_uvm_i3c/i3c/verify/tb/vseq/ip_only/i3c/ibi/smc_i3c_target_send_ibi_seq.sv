...
//
// File        : smc_i3c_target_send_ibi_seq.sv
...
`ifndef SMC_I3C_TARGET_SEND_IBI_SEQ_SV
`define SMC_I3C_TARGET_SEND_IBI_SEQ_SV

class smc_i3c_target_send_ibi_seq extends
    uvm_sequence #(i3c_seq_item, i3c_seq_item);
  `uvm_object_utils(smc_i3c_target_send_ibi_seq)
  ...
  function new(string name = "smc_i3c_target_send_ibi_seq");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    if (byte_count == 0)
      `uvm_fatal("IBI_TARGET_EMPTY",
                 "Controller receive test requires at least an MDB")
  ...
  endtask
endclass : smc_i3c_target_send_ibi_seq

`endif // SMC_I3C_TARGET_SEND_IBI_SEQ_SV
