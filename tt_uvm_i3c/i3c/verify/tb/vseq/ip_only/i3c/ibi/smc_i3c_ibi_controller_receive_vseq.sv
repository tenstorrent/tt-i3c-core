...
//
// File        : smc_i3c_ibi_controller_receive_vseq.sv
...
`ifndef SMC_I3C_IBI_CONTROLLER_RECEIVE_VSEQ_SV
`define SMC_I3C_IBI_CONTROLLER_RECEIVE_VSEQ_SV

class smc_i3c_ibi_controller_receive_vseq extends smc_i3c_ibi_base_vseq;
  `uvm_object_utils(smc_i3c_ibi_controller_receive_vseq)
  ...
  function new(string name = "smc_i3c_ibi_controller_receive_vseq");
    super.new(name);
  endfunction
...
`ifdef SMC_USE_I3C_RAL
    begin
      ...
      wait_scoreboard_completion(
        1, "wait for Controller IBI scoreboard completion");
      ...
      if ((vseq_ctx.cfg.ibi_cov_attempt_samples == 0) ||
          (vseq_ctx.cfg.ibi_cov_completion_samples == 0) ||
          (vseq_ctx.cfg.ibi_cov_queue_irq_samples == 0))
        `uvm_fatal("IBI_CTRL_COVERAGE",
                   $sformatf("Missing Controller IBI coverage events attempt=%0d completion=%0d queue_irq=%0d",
...
`endif
  endtask
endclass : smc_i3c_ibi_controller_receive_vseq

`endif // SMC_I3C_IBI_CONTROLLER_RECEIVE_VSEQ_SV
