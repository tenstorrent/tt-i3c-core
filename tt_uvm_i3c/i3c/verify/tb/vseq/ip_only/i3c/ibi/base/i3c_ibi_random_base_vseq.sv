...
//
// File        : i3c_ibi_random_base_vseq.sv
...
`ifndef I3C_IBI_RANDOM_BASE_VSEQ_SV
`define I3C_IBI_RANDOM_BASE_VSEQ_SV

class i3c_ibi_random_base_vseq extends i3c_ibi_base_vseq;
  `uvm_object_utils(i3c_ibi_random_base_vseq)

  function new(string name = "i3c_ibi_random_base_vseq");
    super.new(name);
  endfunction
...
`ifdef I3C_DV_NATIVE_RAL
  protected task write_random_descriptor(
      ...
      input i3c_ibi_random_item item,
  ...
  protected task wait_queue_reset_clear(input i3c_ral_model ral,
                                        input string phase_name);
  ...
  protected task run_direct_item(input i3c_ral_model ral,
                                 input i3c_ibi_random_item item,
                                 input string phase_name,
                                 input bit post_recovery_check = 1'b0);
    i3c_host_respond_ibi_seq host_seq;
    ...
    host_seq = i3c_host_respond_ibi_seq::type_id::create(
  ...
  protected task run_retry_item(input i3c_ral_model ral,
                                input i3c_ibi_random_item item,
                                input string phase_name);
...
`endif
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("IBI_RANDOM_MODE", "Random IBI test requires native I3C RTL mode")
...
`ifndef I3C_DV_NATIVE_RTL
    `uvm_fatal("IBI_RANDOM_RTL_BUILD",
               "Random IBI test requires I3C_DV_NATIVE_RTL")
`elsif I3C_DV_NATIVE_RAL
    begin
      i3c_ral_model ral;
      i3c_ibi_random_plan plan;
      ...
      plan = i3c_ibi_random_plan::type_id::create("plan");
...
`else
    `uvm_fatal("IBI_RANDOM_RAL_BUILD",
               "Random IBI test requires I3C_DV_NATIVE_RAL")
`endif
  endtask
endclass : i3c_ibi_random_base_vseq

`endif // I3C_IBI_RANDOM_BASE_VSEQ_SV
