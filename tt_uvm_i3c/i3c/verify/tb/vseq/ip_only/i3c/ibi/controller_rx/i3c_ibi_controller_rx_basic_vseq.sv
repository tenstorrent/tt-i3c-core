...
//
// File        : i3c_ibi_controller_rx_basic_vseq.sv
...
`ifndef I3C_IBI_CONTROLLER_RX_BASIC_VSEQ_SV
`define I3C_IBI_CONTROLLER_RX_BASIC_VSEQ_SV

class i3c_ibi_controller_rx_basic_vseq extends i3c_ibi_base_vseq;
  `uvm_object_utils(i3c_ibi_controller_rx_basic_vseq)
  ...
  //                 when start_on_idle=0 (i3c_dv_target_driver.sv:90), so this
  ...
  function new(string name = "i3c_ibi_controller_rx_basic_vseq");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("IBI_CTRL_MODE",
                 "Controller receive test requires native I3C RTL mode")
...
`else
    `uvm_fatal("IBI_CTRL_RAL_BUILD",
               "Controller receive test requires I3C_DV_NATIVE_RAL")
`endif
`else
    `uvm_fatal("IBI_CTRL_RTL_BUILD",
               "Controller receive test requires I3C_DV_NATIVE_RTL")
`endif
  endtask
endclass : i3c_ibi_controller_rx_basic_vseq

`endif // I3C_IBI_CONTROLLER_RX_BASIC_VSEQ_SV
