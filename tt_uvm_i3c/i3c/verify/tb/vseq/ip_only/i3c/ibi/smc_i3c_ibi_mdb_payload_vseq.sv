...
//
// File        : smc_i3c_ibi_mdb_payload_vseq.sv
...
`ifndef SMC_I3C_IBI_MDB_PAYLOAD_VSEQ_SV
`define SMC_I3C_IBI_MDB_PAYLOAD_VSEQ_SV

class smc_i3c_ibi_mdb_payload_vseq extends smc_i3c_ibi_base_vseq;
  `uvm_object_utils(smc_i3c_ibi_mdb_payload_vseq)

  function new(string name = "smc_i3c_ibi_mdb_payload_vseq");
    super.new(name);
  endfunction
...
`ifdef SMC_USE_I3C_RAL
  ...
  protected task run_payload_ibi(input smc_i3c_ral_model ral,
    ...
    expected_status = IBI_STATUS_SUCCESS;
    ...
    // Host samples MDB followed by every payload byte.
    host_seq.rx_byte_count = payload_len + 1;
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("SMC_I3C_MODE",
                 "smc_i3c_ibi_mdb_payload_test requires DUT_MODE=rtl")
    if (vseq_ctx.cfg.ibi_dut_role != SMC_I3C_IBI_DUT_TARGET)
      `uvm_fatal("IBI_PAYLOAD_ROLE",
                 "smc_i3c_ibi_mdb_payload_test covers DUT-target transmit only")
...
`elsif SMC_USE_I3C_RAL
    begin
      ...
      // short {2:4}, medium {5:8}, long {9:$} payload_len coverage bins.
      int unsigned payload_lengths[3] = '{3, 6, 10};
      ...
      foreach (payload_lengths[i])
        run_payload_ibi(ral, payload_lengths[i],
                        $sformatf("PAYLOAD_%0d", payload_lengths[i]));
      ...
      if ((attempt_samples < $size(payload_lengths)) ||
          (completion_samples < $size(payload_lengths)) ||
          (queue_irq_samples < $size(payload_lengths))) begin
        if (vseq_ctx.cfg.require_ibi_coverage_samples)
          `uvm_fatal("IBI_COVERAGE_WIRING",
                     $sformatf("Payload coverage is incomplete: attempt=%0d completion=%0d queue_irq=%0d expected>=%0d",
                               ...
                               queue_irq_samples, $size(payload_lengths)))
        else
          `uvm_warning("IBI_COVERAGE_WIRING",
                       $sformatf("Payload functional checks passed with incomplete coverage observation: attempt=%0d completion=%0d queue_irq=%0d",
                                 attempt_samples, completion_samples,
                                 queue_irq_samples))
      end else if ((attempt_samples > $size(payload_lengths)) ||
                   (completion_samples > $size(payload_lengths))) begin
      ...
      `uvm_info("IBI_MDB_PAYLOAD_PASS",
                $sformatf("DUT target transmitted MDB+payload IBIs (lengths=%p): DA=0x%02h MDB=0x%02h",
                          payload_lengths, vseq_ctx.cfg.ibi_target_addr,
                          vseq_ctx.cfg.ibi_basic_mdb),
                UVM_LOW)
    end
`else
    `uvm_fatal("IBI_RAL_BUILD",
               "smc_i3c_ibi_mdb_payload_test requires SMC_USE_I3C_RAL")
`endif
  endtask
endclass : smc_i3c_ibi_mdb_payload_vseq

`endif // SMC_I3C_IBI_MDB_PAYLOAD_VSEQ_SV
