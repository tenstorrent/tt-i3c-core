...
//
// File        : i3c_ibi_target_tx_ack_nack_vseq.sv
...
`ifndef I3C_IBI_TARGET_TX_ACK_NACK_VSEQ_SV
`define I3C_IBI_TARGET_TX_ACK_NACK_VSEQ_SV

class i3c_ibi_target_tx_ack_nack_vseq extends i3c_ibi_base_vseq;
  `uvm_object_utils(i3c_ibi_target_tx_ack_nack_vseq)

  function new(string name = "i3c_ibi_target_tx_ack_nack_vseq");
    super.new(name);
  endfunction
...
`endif
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("I3C_MODE",
                 "i3c_ibi_target_tx_ack_nack_test requires native I3C RTL mode")
    if (vseq_ctx.cfg.ibi_dut_role != I3C_IBI_DUT_TARGET)
      `uvm_fatal("IBI_ACK_NACK_ROLE",
                 "i3c_ibi_target_tx_ack_nack_test covers DUT-target transmit only")
...
`else
    `uvm_fatal("IBI_RAL_BUILD",
               "i3c_ibi_target_tx_ack_nack_test requires I3C_DV_NATIVE_RAL")
`endif
  endtask
endclass : i3c_ibi_target_tx_ack_nack_vseq

`endif // I3C_IBI_TARGET_TX_ACK_NACK_VSEQ_SV
