...
//
// File        : smc_i3c_ibi_controller_rx_fifo_irq_vseq.sv
...
`ifndef SMC_I3C_IBI_CONTROLLER_RX_FIFO_IRQ_VSEQ_SV
`define SMC_I3C_IBI_CONTROLLER_RX_FIFO_IRQ_VSEQ_SV

class smc_i3c_ibi_controller_rx_fifo_irq_vseq extends smc_i3c_ibi_base_vseq;
  `uvm_object_utils(smc_i3c_ibi_controller_rx_fifo_irq_vseq)

  // The pinned RTL applies this threshold to IBI_PORT DWORD occupancy. Each
  ...
  function new(string name = "smc_i3c_ibi_controller_rx_fifo_irq_vseq");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("IBI_CTRL_FIFO_MODE",
                 "Controller FIFO/IRQ test requires DUT_MODE=rtl")
...
`else
    `uvm_fatal("IBI_CTRL_FIFO_RAL_BUILD",
               "Controller FIFO/IRQ test requires SMC_USE_I3C_RAL")
`endif
`else
    `uvm_fatal("IBI_CTRL_FIFO_RTL_BUILD",
               "Controller FIFO/IRQ test requires SMC_USE_I3C_RTL")
`endif
  endtask
endclass : smc_i3c_ibi_controller_rx_fifo_irq_vseq

`endif // SMC_I3C_IBI_CONTROLLER_RX_FIFO_IRQ_VSEQ_SV
