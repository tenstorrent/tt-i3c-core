...
//
// File        : i3c_ibi_target_tx_recovery_cleanup_vseq.sv
...
`ifndef I3C_IBI_TARGET_TX_RECOVERY_CLEANUP_VSEQ_SV
`define I3C_IBI_TARGET_TX_RECOVERY_CLEANUP_VSEQ_SV

class i3c_ibi_target_tx_recovery_cleanup_vseq extends i3c_ibi_target_tx_ack_nack_vseq;
  `uvm_object_utils(i3c_ibi_target_tx_recovery_cleanup_vseq)

  function new(string name = "i3c_ibi_target_tx_recovery_cleanup_vseq");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("IBI_RECOVERY_MODE",
                 "Target recovery cleanup requires native I3C RTL mode")
...
`else
    `uvm_fatal("IBI_RECOVERY_RAL_BUILD",
               "Target recovery cleanup requires I3C_DV_NATIVE_RAL")
`endif
`else
    `uvm_fatal("IBI_RECOVERY_RTL_BUILD",
               "Target recovery cleanup requires I3C_DV_NATIVE_RTL")
`endif
  endtask
endclass : i3c_ibi_target_tx_recovery_cleanup_vseq

`endif // I3C_IBI_TARGET_TX_RECOVERY_CLEANUP_VSEQ_SV
