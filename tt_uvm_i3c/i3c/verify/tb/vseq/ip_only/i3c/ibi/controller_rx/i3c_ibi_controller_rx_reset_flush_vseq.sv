...
//
// File        : i3c_ibi_controller_rx_reset_flush_vseq.sv
...
`ifndef I3C_IBI_CONTROLLER_RX_RESET_FLUSH_VSEQ_SV
`define I3C_IBI_CONTROLLER_RX_RESET_FLUSH_VSEQ_SV
...
class i3c_ibi_controller_rx_reset_flush_vseq extends i3c_ibi_base_vseq;
  `uvm_object_utils(i3c_ibi_controller_rx_reset_flush_vseq)

  function new(string name = "i3c_ibi_controller_rx_reset_flush_vseq");
    super.new(name);
  endfunction
...
`ifdef I3C_DV_NATIVE_RAL
  ...
  protected task clear_ibi_queue(i3c_ral_model ral, output bit self_cleared);
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("IBI_QUEUE_CLEAR_MODE",
                 "Controller IBI queue-clear test requires native I3C RTL mode")
...
`ifdef I3C_DV_NATIVE_RAL
    begin
      ...
      // destructive pop (i3c_controller_hci_helper.sv:339) and draining the
...
`else
    `uvm_fatal("IBI_QUEUE_CLEAR_RAL_BUILD",
               "Controller IBI queue-clear test requires I3C_DV_NATIVE_RAL")
`endif
`else
    `uvm_fatal("IBI_QUEUE_CLEAR_RTL_BUILD",
               "Controller IBI queue-clear test requires I3C_DV_NATIVE_RTL")
`endif
  endtask
endclass : i3c_ibi_controller_rx_reset_flush_vseq

`endif // I3C_IBI_CONTROLLER_RX_RESET_FLUSH_VSEQ_SV
