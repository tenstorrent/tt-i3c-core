...
//
// File        : i3c_ibi_target_tx_mdb_payload_vseq.sv
...
`ifndef I3C_IBI_TARGET_TX_MDB_PAYLOAD_VSEQ_SV
`define I3C_IBI_TARGET_TX_MDB_PAYLOAD_VSEQ_SV

class i3c_ibi_target_tx_mdb_payload_vseq extends i3c_ibi_base_vseq;
  `uvm_object_utils(i3c_ibi_target_tx_mdb_payload_vseq)

  function new(string name = "i3c_ibi_target_tx_mdb_payload_vseq");
    super.new(name);
  endfunction
...
`ifdef I3C_DV_NATIVE_RAL
  ...
  protected task run_payload_ibi(input i3c_ral_model ral,
...
`endif
  ...
  virtual task body();
    ...
    if (!vseq_ctx.cfg.has_rtl)
      `uvm_fatal("I3C_MODE",
                 "i3c_ibi_target_tx_mdb_payload_test requires native I3C RTL mode")
    if (vseq_ctx.cfg.ibi_dut_role != I3C_IBI_DUT_TARGET)
      `uvm_fatal("IBI_PAYLOAD_ROLE",
                 "i3c_ibi_target_tx_mdb_payload_test covers DUT-target transmit only")
`ifndef I3C_DV_NATIVE_RTL
    `uvm_fatal("IBI_VIP_BUILD", "I3C host VIP is available only in native I3C RTL mode")
...
`else
    `uvm_fatal("IBI_RAL_BUILD",
               "i3c_ibi_target_tx_mdb_payload_test requires I3C_DV_NATIVE_RAL")
`endif
  endtask
endclass : i3c_ibi_target_tx_mdb_payload_vseq

`endif // I3C_IBI_TARGET_TX_MDB_PAYLOAD_VSEQ_SV
