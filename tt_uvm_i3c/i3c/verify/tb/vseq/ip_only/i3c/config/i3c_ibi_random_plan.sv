...
`ifndef I3C_IBI_RANDOM_PLAN_SV
`define I3C_IBI_RANDOM_PLAN_SV

typedef enum bit [1:0] {
  IBI_RANDOM_ACK,
  IBI_RANDOM_NACK_FLUSH,
  IBI_RANDOM_RETRY_ACK
} i3c_ibi_random_response_e;
...
} i3c_ibi_random_policy_e;

class i3c_ibi_random_item extends uvm_object;
  `uvm_object_utils(i3c_ibi_random_item)

  rand bit                              mdb_present;
  rand bit [7:0]                        mdb;
  rand i3c_ibi_random_mdb_class_e   mdb_class;
  rand int unsigned                     payload_len;
  rand i3c_ibi_random_response_e    response_mode;
  rand int unsigned                     nack_count_before_ack;
  rand int unsigned                     inter_ibi_gap;
  rand i3c_ibi_random_policy_e      controller_policy;
  ...
  constraint c_role {
    if (controller_profile) {
      ...
      // The bundled/reviewed I3C VIP cannot launch an empty/no-MDB IBI. Keep the
  ...
  function new(string name = "i3c_ibi_random_item");
    super.new(name);
  endfunction
  ...
  function bit validate(output string reason);
    ...
    if (!mdb_present) begin
      reason = "bundled/reviewed I3C VIP requires an MDB";
      return 1'b0;
    end
...
endclass : i3c_ibi_random_item

class i3c_ibi_random_plan extends uvm_object;
  `uvm_object_utils(i3c_ibi_random_plan)

  i3c_ibi_random_item items[$];

  function new(string name = "i3c_ibi_random_plan");
    super.new(name);
  endfunction
  ...
  function void build(int unsigned item_count,
    ...
    for (int unsigned index = 0; index < item_count; index++) begin
      item = i3c_ibi_random_item::type_id::create(
...
endclass : i3c_ibi_random_plan

`endif // I3C_IBI_RANDOM_PLAN_SV
