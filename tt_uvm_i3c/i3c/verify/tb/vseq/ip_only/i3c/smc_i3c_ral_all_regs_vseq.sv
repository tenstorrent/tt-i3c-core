...
//
// File        : smc_i3c_ral_all_regs_vseq.sv
...
class smc_i3c_ral_all_regs_vseq extends smc_peripherals_base_vseq;
    `uvm_object_utils(smc_i3c_ral_all_regs_vseq)
    ...
    function new(string name = "smc_i3c_ral_all_regs_vseq");
        super.new(name);
    endfunction
    ...
    protected task audit_register(uvm_reg register_handle,
        ...
        if (!readable || destructive_read || has_read_side_effect(full_name)) begin
            read_skip_count++;
            `uvm_info("SMC_I3C_RAL_SKIP_READ", $sformatf(
            ...
            if (status != UVM_IS_OK) begin
                record_failure("SMC_I3C_RAL_READ", $sformatf(
                ...
                if (reset_mask != '0) begin
                    ...
                    if ((saved_data & reset_mask) !== (reset_value & reset_mask))
                        record_failure("SMC_I3C_RAL_RESET", $sformatf(
        ...
        if (!read_completed || writable_mask == '0 || unsafe_write_access ||
            ...
            write_skip_count++;
            `uvm_info("SMC_I3C_RAL_SKIP_WRITE", $sformatf(
        ...
        for (int unsigned pattern_index = 0; pattern_index < 2; pattern_index++) begin
            ...
            if (status != UVM_IS_OK) begin
                record_failure("SMC_I3C_RAL_WRITE", $sformatf(
            ...
            if (status != UVM_IS_OK) begin
                record_failure("SMC_I3C_RAL_READBACK", $sformatf(
        ...
        if (status != UVM_IS_OK)
            record_failure("SMC_I3C_RAL_RESTORE", $sformatf(
    ...
    protected function void report_summary();
        `uvm_info("SMC_I3C_RAL", "-------------------------------------------------", UVM_LOW)
        `uvm_info("SMC_I3C_RAL", " I3C FULL RAL REGISTER AUDIT SUMMARY", UVM_LOW)
        `uvm_info("SMC_I3C_RAL", $sformatf(" Registers inventoried : %0d", register_count), UVM_LOW)
        `uvm_info("SMC_I3C_RAL", $sformatf(" Memories inventoried  : %0d (reported, not 32-bit tested)", memory_count), UVM_LOW)
        `uvm_info("SMC_I3C_RAL", $sformatf(" Read attempts         : %0d", read_attempt_count), UVM_LOW)
        `uvm_info("SMC_I3C_RAL", $sformatf(" Read skips            : %0d", read_skip_count), UVM_LOW)
        `uvm_info("SMC_I3C_RAL", $sformatf(" Reset-value checks    : %0d", reset_check_count), UVM_LOW)
        `uvm_info("SMC_I3C_RAL", $sformatf(" RW registers tested   : %0d", write_register_count), UVM_LOW)
        `uvm_info("SMC_I3C_RAL", $sformatf(" RW pattern checks     : %0d", write_check_count), UVM_LOW)
        `uvm_info("SMC_I3C_RAL", $sformatf(" Write skips           : %0d", write_skip_count), UVM_LOW)
        `uvm_info("SMC_I3C_RAL", $sformatf(" Registers restored    : %0d", restore_count), UVM_LOW)
        `uvm_info("SMC_I3C_RAL", $sformatf(" Failures              : %0d", failure_count), UVM_LOW)
        if (failure_count == 0)
            `uvm_info("SMC_I3C_RAL", " PASSED: all modeled safe accesses matched", UVM_LOW)
        else
            `uvm_error("SMC_I3C_RAL", $sformatf(
    ...
    virtual task body();
        ...
        if (!vseq_ctx.cfg.has_rtl)
            `uvm_fatal("SMC_I3C_RAL_MODE", "RAL all-register test requires DUT_MODE=rtl")
        ...
        if (register_count < MIN_EXPECTED_REGISTERS)
            `uvm_fatal("SMC_I3C_RAL_INVENTORY", $sformatf(
        ...
        if (register_count != (read_attempt_count + read_skip_count))
            record_failure("SMC_I3C_RAL_ACCOUNTING", $sformatf(
        ...
        report_summary();
    endtask
endclass : smc_i3c_ral_all_regs_vseq
