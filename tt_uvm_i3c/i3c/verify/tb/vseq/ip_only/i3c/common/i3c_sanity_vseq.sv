...
class i3c_sanity_vseq extends i3c_base_vseq;
    `uvm_object_utils(i3c_sanity_vseq)
    ...
    function new(string name = "i3c_sanity_vseq");
        super.new(name);
    endfunction
    ...
    protected task run_one_byte_private_write();
        i3c_csr_helper #(TB_ADDR_WIDTH, TB_DATA_WIDTH) helper;
        bit completed;
        ...
        if (vseq_ctx.cfg.vif == null)
            `uvm_fatal("I3C_SANITY_BUS", "Missing I3C sanity target BFM interface")
        ...
        if (read_response != 2'b00 || write_response != 2'b00) begin
            `uvm_error("I3C_SANITY_ENABLE", $sformatf(
        ...
        if (!completed) begin
            ...
            if (observed_address_byte !== {SANITY_TARGET_ADDRESS, 1'b0})
                `uvm_error("I3C_SANITY_ADDRESS", $sformatf(
            ...
            if (vseq_ctx.cfg.vif.sanity_scl_rise_count != 18)
                `uvm_error("I3C_SANITY_SCL_COUNT", $sformatf(
            ...
            if (response_port_response != 2'b00)
                ...
                if (response_descriptor[27:24] !== {1'b0, SANITY_TRANSACTION_ID})
                    `uvm_error("I3C_SANITY_RESPONSE_TID", $sformatf(
                ...
                if (response_descriptor[15:0] !== 16'd0)
                    `uvm_error("I3C_SANITY_RESPONSE_LENGTH", $sformatf(
        ...
        if (restore_response != 2'b00)
            `uvm_error("I3C_SANITY_RESTORE_CONTROL", $sformatf(
        ...
        if (!controller_restored)
            `uvm_error("I3C_SANITY_RESTORE_MODE",
                       "Failed to restore PIO_CONTROL or STBY_CR_CONTROL")

        `uvm_info("I3C_SANITY_BUS", $sformatf(
    ...
    virtual task body();
        ...
        if (!vseq_ctx.cfg.has_rtl)
            `uvm_fatal("I3C_MODE", "i3c_sanity_test requires native I3C RTL mode")
        ...
        expect_read("I3C_SANITY_VERSION", HCI_VERSION_OFFSET, data);
        if (data !== HCI_VERSION_RESET)
            `uvm_error("I3C_SANITY_VERSION", $sformatf(
                "HCI_VERSION mismatch: expected=0x%08h actual=0x%08h",
                HCI_VERSION_RESET, data))

        expect_read("I3C_SANITY_SAVE", INTR_ENABLE_OFFSET, saved_intr_enable);
        foreach (patterns[i]) begin
            expect_write($sformatf("I3C_SANITY_WRITE_%0d", i),
                         INTR_ENABLE_OFFSET, patterns[i]);
            expect_read($sformatf("I3C_SANITY_READ_%0d", i),
                        INTR_ENABLE_OFFSET, data);
            if ((data & INTR_ENABLE_MASK) !== patterns[i])
                `uvm_error("I3C_SANITY_READBACK", $sformatf(
                    "Pattern[%0d] mismatch: expected=0x%08h actual=0x%08h mask=0x%08h",
                    i, patterns[i], data, INTR_ENABLE_MASK))
        end

        expect_write("I3C_SANITY_RESTORE", INTR_ENABLE_OFFSET,
                     saved_intr_enable & INTR_ENABLE_MASK);
        expect_read("I3C_SANITY_RESTORE_CHECK", INTR_ENABLE_OFFSET, data);
        if ((data & INTR_ENABLE_MASK) !== (saved_intr_enable & INTR_ENABLE_MASK))
            `uvm_error("I3C_SANITY_RESTORE_CHECK", $sformatf(
