...
//
// File        : i3c_ibi_completion_coverage.sv
...
`ifndef I3C_IBI_COMPLETION_COVERAGE_SV
`define I3C_IBI_COMPLETION_COVERAGE_SV

class i3c_ibi_completion_coverage extends uvm_object;
  `uvm_object_utils(i3c_ibi_completion_coverage)
...
`endif
    ...
    x_content: cross cp_mdb_group, cp_payload_len, cp_response {
      ...
      // The reviewed native Controller requires an MDB for every accepted IBI. An
    ...
    x_policy: cross cp_dat_decision, cp_payload_policy, cp_mdb_present {
      ...
      // In the reviewed native Controller model the DAT payload-enable bit gates IBI
...
`endif
  ...
  function new(string name = "i3c_ibi_completion_coverage");
    super.new(name);
...
endclass : i3c_ibi_completion_coverage

`endif // I3C_IBI_COMPLETION_COVERAGE_SV
