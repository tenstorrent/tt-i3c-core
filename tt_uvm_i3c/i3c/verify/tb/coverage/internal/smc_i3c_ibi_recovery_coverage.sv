...
//
// File        : smc_i3c_ibi_recovery_coverage.sv
...
`ifndef SMC_I3C_IBI_RECOVERY_COVERAGE_SV
`define SMC_I3C_IBI_RECOVERY_COVERAGE_SV

class smc_i3c_ibi_recovery_coverage extends uvm_object;
  `uvm_object_utils(smc_i3c_ibi_recovery_coverage)
  ...
  covergroup cg_ibi_recovery;
    ...
    cp_cause: coverpoint cov_cause {
      ...
      // The pinned RTL exposes no architectural IBI-abort recovery result.
  ...
  function new(string name = "smc_i3c_ibi_recovery_coverage");
    super.new(name);
    cg_ibi_recovery = new();
  endfunction

  function void sample_recovery(smc_i3c_ibi_item item);
    cov_role = item.role;
...
endclass : smc_i3c_ibi_recovery_coverage

`endif // SMC_I3C_IBI_RECOVERY_COVERAGE_SV
