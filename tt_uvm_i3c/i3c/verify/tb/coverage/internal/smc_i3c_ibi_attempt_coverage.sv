...
`ifndef SMC_I3C_IBI_ATTEMPT_COVERAGE_SV
`define SMC_I3C_IBI_ATTEMPT_COVERAGE_SV

class smc_i3c_ibi_attempt_coverage extends uvm_object;
  `uvm_object_utils(smc_i3c_ibi_attempt_coverage)

  // arb_outcome / bus_state encodings (mirror smc_i3c_ibi_item).
...
`endif
  ...
  function new(string name = "smc_i3c_ibi_attempt_coverage");
    super.new(name);
...
endclass : smc_i3c_ibi_attempt_coverage

`endif // SMC_I3C_IBI_ATTEMPT_COVERAGE_SV
