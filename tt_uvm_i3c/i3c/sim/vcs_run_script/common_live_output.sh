...
runner_parent_activity_snapshot() {
    ...
    compile_log="$attempt_root/logs/compile/compile.log"
    compile_meta="$attempt_root/compile.meta"
    if [[ ! -f "$compile_meta" ]]; then
        phase=compile
        size="$(runner_file_size_bytes "$compile_log")"
    elif [[ -d "$attempt_root/cases" ]]; then
        phase=run
        while IFS= read -r -d '' file; do
            size=$((size + $(runner_file_size_bytes "$file")))
        done < <(find "$attempt_root/cases" -type f -name 'sim.log' -print0 2>/dev/null)
    else
        phase=setup
    fi
...
runner_wait_pid_with_parent_progress() {
    ...
    local interval="${RUNNER_HEARTBEAT_SEC:-1}" last=0 now
...
runner_execute_logged() {
    ...
    local scheduler="${RUNNER_ACTIVE_SCHEDULER:-local}" job_name="${RUNNER_ACTIVE_JOB_NAME:-}" job_id='' raw_job_state='' snapshot=''
    ...
    while kill -0 "$pid" 2>/dev/null; do
        ...
        if [[ "$scheduler" == slurm && -n "$job_name" ]]; then
            ...
            if [[ -n "$snapshot" ]]; then
                IFS='|' read -r job_id raw_job_state <<< "$snapshot"
        ...
        if ((suspect_since > 0)); then
            ...
            if [[ "$scheduler" == slurm && "$scheduler_state" == RUNNING && "${RUNNER_ALLOW_REMOTE_HANG_KILL:-0}" != 1 ]]; then effective_kill=0; fi
