...
runner_run_test_seed() {
    ...
    while (( attempt < TEST_SEED_MAX_ATTEMPTS )); do
        ...
        case_dir="$TEST_RUN_DIR/tests/$TEST_NAME/seed_$seed/attempt_$(printf '%03d' "$attempt")"
        ...
        elif [[ -z "$TEST_BURST_ID" ]]; then
            printf '[START] T001 A%03d - S=%-10s T=%-60s C=-\n' \
                "$attempt" "$seed" "$TEST_NAME"
        fi
...
runner_test_compile_requirements() {
    local test_name="$1" file="${TEST_LIST_FILE:-}" version name case_id group level enabled seed_policy seeds plusargs tags requirements
    [[ -r "$file" ]] || return 0
    version="$(sed -n 's/^#[[:space:]]*DV_TESTPLAN_VERSION=\([0-9][0-9]*\).*/\1/p' "$file" | head -1)"
    while IFS='|' read -r name case_id group level enabled seed_policy seeds plusargs tags requirements; do
        if [[ "$version" == 1 ]]; then
            requirements="$tags"
        fi
...
runner_test_compile_options() {
    local requirements="$1" requirement options=''
    runner_split_csv "$requirements"
    for requirement in "${RUNNER_SPLIT_RESULT[@]}"; do
        case "$requirement" in
            '') ;;
            hdl_access) options+=" -debug_access+all" ;;
            *)
                runner_error "Unknown compile requirement '$requirement'"
                return 2
                ;;
        esac
    done
...
runner_run_test() {
    ...
    local compile_requirements_override='' compile_requirements_override_set=0
    ...
    while (($#)); do
        case "$1" in
            ...
            --compile-requirements) compile_requirements_override="$2"; compile_requirements_override_set=1; shift 2 ;;
    ...
    local metadata_dir="$run_dir/metadata" run_meta="$run_dir/metadata/run.meta"
    mkdir -p "$build_dir" "$log_dir" "$summary_dir" "$cov_dir" "$metadata_dir"
    ...
    local test_start compile_rc=0 compile_reason compile_requirements compile_requirement_opts effective_compile_opts
    if ((compile_requirements_override_set)); then
        compile_requirements="$compile_requirements_override"
    else
        # Direct tests do not pass through the regression planner. Refresh the
        # normalized plan so a newly added requirement cannot be hidden by a
        # stale generated file after checkout or pull.
        runner_project_generate >/dev/null || return $?
        compile_requirements="$(runner_test_compile_requirements "$test_name")"
    fi
    compile_requirement_opts="$(runner_test_compile_options "$compile_requirements")" || return $?
    effective_compile_opts="${DV_EXTRA_COMPILE_OPTS:-${EXTRA_COMPILE_OPTS:-}}${compile_requirement_opts:+ $compile_requirement_opts}"
    test_start="$(date +%s)"
    ...
    printf 'run_id=%s\nkind=test\ntest=%s\nseeds=%s\njobs=%s\nscheduler=%s\nsimulation_cpus=%s\ncompile_jobs=%s\ncompile_policy=private\ncompile_requirements=%s\ncreated=%s\n' \
        "$run_id" "$test_name" "$(IFS=,; echo "${seeds[*]}")" "$jobs" "$scheduler" "$cpus" "$compile_jobs" \
        "${compile_requirements:-none}" "$(date -Is)" > "$run_meta"
    ...
    if runner_test_compact_enabled; then
        ...
        for seed in "${seeds[@]}"; do
            printf '[PLAN] T001 - S=%-10s T=%-60s C=-\n' \
                "$seed" "$test_name"
        done
        printf '%s\n' '--------------------------------------------------------------------------------'
    fi
    ...
    if ((do_compile)); then
        [[ -z "$compile_requirements" ]] || printf '[COMPILE REQUIREMENT] test=%s requires=%s\n' \
            "$test_name" "$compile_requirements"
        local -a compile_args=(--scheduler "$scheduler" --cpus "$compile_jobs" --timeout "$timeout_sec"
            ...
            --build-dir "$build_dir" --log-dir "$log_dir" --cov-dir "$cov_dir" --dump-dir "$dump_dir"
        ...
        DV_EXTRA_COMPILE_OPTS="$effective_compile_opts" EXTRA_COMPILE_OPTS="$effective_compile_opts" \
            runner_run_compile "${compile_args[@]}" || compile_rc=$?
        if ((compile_rc != 0)); then
            compile_reason="$(runner_failure_reason "$compile_rc" "$log_dir/compile.log")"
            case "$compile_reason" in
                TOOL_FAIL|INFRA_FAIL|LICENSE_FAIL|DISK_FAIL|SCHEDULER_FAIL|TIMEOUT) ;;
                *) compile_reason=COMPILE_FAIL ;;
            esac
            for seed in "${seeds[@]}"; do
                runner_append_seed_summary "$seed|A000|FAIL|$compile_rc|$compile_reason|0|$log_dir/compile.log|0|0|0|0|0"
            done
            printf 'status=%s\ncompleted=%s\n' "$compile_rc" "$(date -Is)" >> "$run_meta"
    ...
    for seed in "${seeds[@]}"; do
        ...
        runner_test_compact_enabled || runner_info "[DISPATCH $dispatched/$total] test=$test_name seed=$seed pid=$pid log=$run_dir/tests/$test_name/seed_$seed/attempt_001/logs/sim.log"
    ...
    printf 'status=%s\ncompleted=%s\n' "$overall" "$(date -Is)" >> "$run_meta"
    runner_report_test "$run_id" "$test_name" "$run_dir" "$TEST_SUMMARY_FILE" "$test_start" "$include_warnings"
    if runner_test_compact_enabled; then
        if ((overall == 0)); then runner_status_token PASS; printf ' TEST run=%s test=%s seeds=%s\n' "$run_id" "$test_name" "$TEST_TOTAL_SEEDS"
        else runner_status_token FAIL; printf ' TEST run=%s test=%s seeds=%s summary=%s\n' "$run_id" "$test_name" "$TEST_TOTAL_SEEDS" "$(runner_report_path "$run_dir/summary/test.errors.log")"
        fi
