...
//
// AXI4-Lite boundary adapter for the external TT I3C core.

`timescale 1ns/1ps

module smc_i3c_axi_adapter #(
    ...
    function automatic logic address_is_legal(input logic [HOST_ADDR_WIDTH-1:0] addr);
        logic [HOST_ADDR_WIDTH:0] window_end;
        window_end = {1'b0, BASE_ADDR} + WINDOW_BYTES;
        return ({1'b0, addr} >= {1'b0, BASE_ADDR}) &&
               ({1'b0, addr} < window_end) &&
               ((addr % CORE_STRB_WIDTH) == 0);
    endfunction

    function automatic logic [CORE_ADDR_WIDTH-1:0] local_address(
        input logic [HOST_ADDR_WIDTH-1:0] addr
    );
        logic [HOST_ADDR_WIDTH-1:0] offset;
        offset = addr - BASE_ADDR;
        return offset[CORE_ADDR_WIDTH-1:0];
    endfunction
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        ...
        if (!rst_ni) begin
            ...
            if (have_aw_q && have_w_q && !issue_aw_q && !issue_w_q && !wait_b_q && !s_bvalid_o) begin
                have_aw_q <= 1'b0;
                have_w_q  <= 1'b0;
                if (!access_allow_i || !address_is_legal(awaddr_q)) begin
                    s_bresp_o  <= RESP_DECERR;
                    s_bvalid_o <= 1'b1;
                end else if (!strobe_is_legal(awaddr_q, wstrb_q)) begin
            ...
            if (s_arvalid_i && s_arready_o) begin
                ...
                if (!access_allow_i || !address_is_legal(s_araddr_i)) begin
                    s_rdata_o  <= '0;
