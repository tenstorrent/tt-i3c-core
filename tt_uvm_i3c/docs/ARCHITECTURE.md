...
- `docs/` records the architecture, qualified source baseline, and known
  limitations.
...
`run.sh` at the package root is the only public execution entry point. The
internal `smc_*` names are retained from the qualified OCAH integration
baseline and do not introduce a dependency on another OCAH directory.
