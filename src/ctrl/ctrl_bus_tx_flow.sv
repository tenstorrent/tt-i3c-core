...
* The `abort_i` cancels request and releases the bus immediately TODO: implement abort_i
* Before asserting a request, ensure `bus_tx_idle_o` is HIGH

*/
module ctrl_bus_tx_flow (
    input logic clk_i,
    input logic rst_ni,
    ...
    input  logic sda_hold_i,  // actual bus value to hold while awaiting SCL negedge
    input  logic release_i,   // release SDA at the bit's SCL negedge (handoff cue)
